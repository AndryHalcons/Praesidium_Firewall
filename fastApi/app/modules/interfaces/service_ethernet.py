"""Validación específica de ethernets, complementaria a service.py.
Ethernet-specific validation, complementary to service.py.

ES: No valida addresses/gateway/routes.to/routes.via/nameservers.addresses porque
validate_alias_fields() ya permite literales y alias.
EN: It does not validate addresses/gateway/routes.to/routes.via/nameservers.addresses
because validate_alias_fields() already supports literals and aliases.
"""
...
ETHERNET_SELECTS = {
    ...
    "infiniband-mode": {"datagram", "connected"},
}


def validate_and_normalize_ethernet_rule(rule: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    """ES/EN: Complementa ethernets sin sustituir validaciones genéricas ni alias."""
    _validate_common_unknown_fields(rule, COMMON_INTERFACE_KEYS | {
        "wakeonlan", "match.name", "match.macaddress", "match.driver", "set-name",
        "virtual-function-count", "embedded-switch-mode", "delay-virtual-functions-rebind", "infiniband-mode",
    }, "ETHERNET")
    _validate_common_netplan_scalars(rule, "ETHERNET")
    if _has(rule, "match.macaddress"):
        _validate_mac_value(_text(rule, "match.macaddress"), "ETHERNET_MATCH_MACADDRESS_INVALID", allow_special=False)
    if _has(rule, "set-name"):
        if not IFACE_RE.match(_text(rule, "set-name")):
            fail("ETHERNET_SET_NAME_INVALID")
        if not any(_has(rule, key) for key in ("match.name", "match.macaddress", "match.driver")):
            fail("ETHERNET_SET_NAME_REQUIRES_MATCH")
    _validate_int(rule, "virtual-function-count", "ETHERNET_VIRTUAL_FUNCTION_COUNT_INVALID", 0)
    _validate_bool(rule, "delay-virtual-functions-rebind", "ETHERNET_DELAY_VF_REBIND_INVALID")
    for key, allowed in ETHERNET_SELECTS.items():
        if _has(rule, key) and _text(rule, key) not in allowed:
            fail(f"ETHERNET_{key.upper().replace('-', '_')}_INVALID")
    return rule
