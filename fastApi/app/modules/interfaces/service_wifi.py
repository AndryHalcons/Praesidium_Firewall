"""Validación específica de Wi-Fi, complementaria a service.py.
Wi-Fi-specific validation, complementary to service.py.

ES: No toca IPs/alias. Valida access-points.* y escalares que el generador Wi-Fi
puede emitir como Netplan inválido.
EN: Does not touch IPs/aliases. Validates access-points.* and scalars that the
Wi-Fi generator can emit as invalid Netplan.
"""
...
WIFI_AP_FIELDS = {"password", "mode", "bssid", "band", "channel", "hidden"}
WIFI_MODES = {"infrastructure", "adhoc"}  # ES: ap requiere NetworkManager; Praesidium genera networkd. EN: ap requires NetworkManager; Praesidium emits networkd.
WIFI_BANDS = {"5GHz", "2.4GHz"}


def validate_and_normalize_wifi_rule(rule: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    """ES/EN: Complementa wifis tras validación genérica."""
    _validate_common_unknown_fields(rule, COMMON_INTERFACE_KEYS | {"wakeonlan", "access-points.*"}, "WIFI")
    _validate_common_netplan_scalars(rule, "WIFI")
    if _has(rule, "wakeonlan"):
        fail("WIFI_WAKEONLAN_NOT_SUPPORTED")
    ssid_fields: dict[str, set[str]] = {}
    for key, value in list(rule.items()):
        if not key.startswith("access-points.") or value in ("", None):
            continue
        parts = key.split(".")
        if len(parts) != 3 or not parts[1].strip() or not parts[2].strip():
            fail("WIFI_ACCESS_POINT_FIELD_INVALID")
        ssid, field = parts[1], parts[2]
        if field not in WIFI_AP_FIELDS:
            fail("WIFI_ACCESS_POINT_FIELD_NOT_SUPPORTED")
        ssid_fields.setdefault(ssid, set()).add(field)
        text = str(value).strip()
        if field == "mode" and text not in WIFI_MODES:
            fail("WIFI_ACCESS_POINT_MODE_INVALID")
        if field == "bssid":
            _validate_mac_value(text, "WIFI_ACCESS_POINT_BSSID_INVALID", allow_special=False)
        if field == "band" and text not in WIFI_BANDS:
            fail("WIFI_ACCESS_POINT_BAND_INVALID")
        if field == "channel":
            if not text.isdigit() or int(text) < 1:
                fail("WIFI_ACCESS_POINT_CHANNEL_INVALID")
            rule[key] = str(int(text))
        if field == "hidden" and _bool_value(value) is None:
            fail("WIFI_ACCESS_POINT_HIDDEN_INVALID")
    for ssid, fields in ssid_fields.items():
        if "channel" in fields and "band" not in fields:
            fail("WIFI_ACCESS_POINT_CHANNEL_REQUIRES_BAND")
    return rule
